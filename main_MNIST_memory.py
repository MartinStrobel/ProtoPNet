...
for seed in range(args.seedL[0],args.seedH[0]):
    ...
    save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + '_' + str(i) + 'push', accu=accu,
                                                target_accu=0.70, log=log)
    prediction_dataloader = DataLoader(train_dataset,batch_size=test_batch_size, shuffle=False,
    ...
    pickle.dump(np.asarray(prediction), open(model_dir+"train_predictions.pkl","wb"))

    test_prediction = tnt.prediction(model=ppnet_multi, dataloader=test_loader,
                        class_specific=class_specific, log=log, evaluation_pred="test.pkl")
    pickle.dump(np.asarray(prediction), open(model_dir+"test_predictions.pkl","wb"))
